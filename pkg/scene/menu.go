...
func (s *MenuScene) Update() (Scene, error) {
	...
	if inpututil.IsMouseButtonJustPressed(ebiten.MouseButtonLeft) {
		...
		case p.In(s.startBtn):
			gs, err := NewGameScene(s.useExtension, s.screenW, s.screenH)
			if err != nil {
				return s, err
			}
			return gs, nil
...
func (s *MenuScene) Draw(screen *ebiten.Image) {
	...
	titleX := s.screenW/2 - len(title)*6/2
	subtitleX := s.screenW/2 - len(subtitle)*6/2
	ebitenutil.DebugPrintAt(screen, title, titleX, s.screenH/2-160)
	ebitenutil.DebugPrintAt(screen, subtitle, subtitleX, s.screenH/2-130)
	...
	ebitenutil.DebugPrintAt(screen, footer, s.screenW/2-len(footer)*6/2, s.screenH-40)
}
